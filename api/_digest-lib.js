...
// One post -> one HTML block, in its own original language/direction.
export function postBlockHtml(p) {
  ...
  return `<div dir="${he ? 'rtl' : 'ltr'}" style="padding:0 0 16px;margin-bottom:16px;border-bottom:1px solid #e5e2d8;text-align:${he ? 'right' : 'left'};">
    ...
    <p style="font-size:14px;color:#5c5c50;line-height:1.6;margin:0 0 4px;">${esc(p.details || '').replace(/\n/g, '<br>')}</p>
    <div style="font-size:11px;color:#8a8a7c;">${byline}</div>
  </div>`;
}
...
export function digestBodyHtml(posts) {
  ...
  return posts.map(postBlockHtml).join('');
}

// Wrap a body in the shared email shell (simple, inline-styled — safe across email clients).
export function wrapEmailHtml({ preheader = '', title, bodyHtml, ctaHtml = '', footerHtml = '' }) {
  return `<div style="font-family:-apple-system,Helvetica,Arial,sans-serif;max-width:560px;margin:0 auto;background:#faf8f2;padding:32px 24px;">
    ...
    <div style="font-family:Georgia,'Playfair Display',serif;font-size:13px;letter-spacing:1px;text-transform:uppercase;color:#c0392b;margin-bottom:4px;">Bama Hub</div>
    <h2 style="font-family:Georgia,'Playfair Display',serif;font-weight:400;font-size:22px;margin:0 0 20px;color:#2b2b24;">${esc(title)}</h2>
